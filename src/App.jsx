...
const FUEL_ICONS = { El: '⚡', Bensin: '⛽', Diesel: '🛢️', Laddhybrid: '🔌' }
...
const DEFAULT_FORDONSSKATT = {
  El: 360, Bensin: 1460, Diesel: 4140, Laddhybrid: 1460,
}

const DEFAULT_FORSAKRING = {
  El: 7200, Bensin: 6000, Diesel: 6500, Laddhybrid: 6500,
}
...
const COST_ICONS = {
  ...
  'Däck och tillbehör': '🔄',
  'Drivmedel': '⛽',
  'Arbetsgivaravgift (förmån)': '🏢',
  'Momsavdrag drivmedel': '✅',
  'Skatteavdrag (20,6%)': '✅',
}
...
function getResidualRate(drivmedel, years) {
  ...
  // Extrapolate beyond 6 years at ~8% per additional year
...
export default function App() {
  ...
  const results = useMemo(() => {
    ...
    const räntekostnad = lanebelopp > 0 && ranta > 0
      ? lanebelopp * (ranta / 100) * år : 0
  ...
  // Bar chart widths relative to largest cost
  const costItems = [
        ...
        <div className="form-col">
          <div className="card">
            ...
            <div className="section">
              <h2 className="section-title">Drivmedel</h2>
              <div className="fuel-grid">
                {['El', 'Bensin', 'Diesel', 'Laddhybrid'].map(d => (
                  <button
                    key={d}
                    className={`fuel-chip ${drivmedel === d ? 'fuel-chip-active' : ''}`}
                    onClick={() => handleDrivmedel(d)}
                  >
                    <span className="fuel-icon">{FUEL_ICONS[d]}</span>
                    <span>{d}</span>
                  </button>
                ))}
              </div>
            ...
            {/* Ägandetid */}
            <div className="section">
              <h2 className="section-title">Ägandetid</h2>
              <div className="year-grid">
                {[1,2,3,4,5,6].map(y => (
                  <button
                    key={y}
                    className={`year-chip ${!customYears && agandetid === y ? 'year-chip-active' : ''}`}
                    onClick={() => { setCustomYears(false); setAgandetid(y) }}
                  >{y} år</button>
                ))}
                <button
                  className={`year-chip year-chip-custom ${customYears ? 'year-chip-active' : ''}`}
            ...
            <div className="section">
              <h2 className="section-title">
                Årlig körsträcka <span className="title-value">{formatNum(korsträcka)} mil</span>
              </h2>
              <input
                type="range"
                className="slider"
            ...
            {/* Driftkostnader */}
            <div className="section">
              <h2 className="section-title">Driftkostnader</h2>
            ...
            <div className="section">
              <h2 className="section-title">Inköpspris och restvärde</h2>
              ...
              <div className="toggle-pills" style={{marginBottom: '16px'}}>
                <button className={`toggle-pill ${autoRestvärde ? 'toggle-pill-active' : ''}`} onClick={() => setAutoRestvärde(true)}>Automatiskt restvärde</button>
              ...
              {autoRestvärde && (
                <div className="depreciation-bar">
                  ...
                  <div className="dep-bar-labels">
                    <span className="dep-label green">Restvärde {Math.round((restvärde / inkopspris) * 100)}%</span>
                    <span className="dep-label red">Värdeminskning {Math.round(((inkopspris - restvärde) / inkopspris) * 100)}%</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Lån */}
            <div className="section">
              <h2 className="section-title">Billån</h2>
              <div className="input-row">
              ...
              <p className="hint">Max 80% av inköpspriset = <strong>{formatNum(maxLan)} kr</strong> för denna bil.</p>
            </div>
          ...
          <div className="results-sticky">
            <div className="results-header">
              <h2 className="results-title">
                Din kostnad över {effectiveYears} år
              </h2>
              <div className="period-switch">
                <button className={`period-btn ${!visaPerManad ? 'period-active' : ''}`} onClick={() => setVisaPerManad(false)}>Totalt</button>
                <button className={`period-btn ${visaPerManad ? 'period-active' : ''}`} onClick={() => setVisaPerManad(true)}>Per mån</button>
              </div>
            </div>
            ...
            <div className="total-box">
              <div className="total-amount">
                ...
                <span className="total-unit">{visaPerManad ? 'kr/mån' : 'kr'}</span>
              </div>
              <div className="total-sub">
                {userType === 'privatperson'
                  ? 'Totalkostnad ink. värdeminskning'
                  : 'Bruttokostnad (privatperson)'}
              </div>
            </div>
              ...
              {costItems.filter(c => c.value > 0).sort((a,b) => b.value - a.value).map(item => (
                <div key={item.label} className="bar-row">
                  <div className="bar-label">
                    <span>{COST_ICONS[item.label]} {item.label}</span>
            ...
            {userType === 'foretag' && (
              <div className="foretag-breakdown">
                ...
                <div className="foretag-net">
                  ...
                  <span className="net-amount">{formatNum(results.nettoFöretagskostnad / divisor)} {visaPerManad ? 'kr/mån' : 'kr'}</span>
                </div>
                {results.anställdSkatteKostnad > 0 && (
                  <div className="foretag-row employee">
